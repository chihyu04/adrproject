...
function App() {
  ...
  useEffect(() => {
    ...
    const pharmacistViews = ['drug_mgmt', 'ref_ranges', 'repository', 'drug_detail'];
    const doctorViews = ['overview', 'dashboard', 'repository', 'drug_detail'];
  ...
  const renderSidebar = () => (
    <div className="w-72 bg-white border-r border-slate-100 flex flex-col p-8 print:hidden">
      ...
      <nav className="flex-1 space-y-2">
        ...
        {isPharmacist && <>
          <button onClick={() => setView('drug_mgmt')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-bold transition-all ${view === 'drug_mgmt' ? 'bg-blue-600 text-white shadow-xl' : 'text-slate-400 hover:bg-slate-50'}`}><Pill size={20} /> 藥物管理</button>
          <button onClick={() => { setView('repository'); setSearchResults([]); }} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-bold transition-all ${view === 'repository' || view === 'drug_detail' ? 'bg-blue-600 text-white shadow-xl' : 'text-slate-400 hover:bg-slate-50'}`}><Search size={20} /> 藥物查詢</button>
          <button onClick={() => setView('ref_ranges')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-bold transition-all ${view === 'ref_ranges' ? 'bg-blue-600 text-white shadow-xl' : 'text-slate-400 hover:bg-slate-50'}`}><SlidersHorizontal size={20} /> 特徵值區間</button>
        </>}
      </nav>
            ...
            {showAddDrug && (
              <div className="bg-white rounded-2xl border border-blue-100 shadow-sm p-6 mb-5">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => setShowAddDrug(false)} className="px-5 py-2 bg-slate-100 text-slate-500 rounded-lg font-bold text-sm hover:bg-slate-200 transition-all">取消</button>
                </div>
              </div>
            )}
                ...
                <table className="w-full text-left">
                  <thead className="bg-slate-50 border-b text-slate-400 text-[11px] font-bold uppercase tracking-widest">
                    <tr>
                      ...
                      <th className="py-5 px-6 text-center w-28">狀態</th>
                      <th className="py-5 px-6 text-center w-20">操作</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {filtered.map((drug, i) => {
                      ...
                      return (
                        <tr key={i} className="hover:bg-blue-50/30 transition-all">
                          ...
                          <td className="py-5 px-6 text-slate-400 font-bold text-sm max-w-sm truncate">{preview}</td>
                          <td className="py-5 px-6 text-center">
                            {hasRx ? <span className="bg-rose-50 text-rose-600 px-3 py-1 rounded-full text-xs font-black border border-rose-200">有不良反應</span>
                                    : <span className="bg-slate-100 text-slate-400 px-3 py-1 rounded-full text-xs font-black">無記載</span>}
                          </td>
                          <td className="py-5 px-6 text-center">
            ...
            {showAddEi && (
              <div className="bg-white rounded-2xl border border-blue-100 shadow-sm p-6 mb-5">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => setShowAddEi(false)} className="px-5 py-2 bg-slate-100 text-slate-500 rounded-lg font-bold text-sm hover:bg-slate-200 transition-all">取消</button>
                </div>
              </div>
            )}
                          ...
                          <tr key={i} className="bg-blue-50/40">
                            ...
                            <td className="py-3 px-4 text-center">
                              <div className="flex justify-center gap-1">
                                ...
                                <button onClick={() => setEditingEiKey(null)} className="px-2.5 py-1 bg-slate-100 text-slate-500 rounded-lg font-bold text-xs hover:bg-slate-200">取消</button>
                              </div>
                            </td>
                          </tr>
                        );
        ...
        {showAddRange && (
          <div className="bg-white rounded-2xl border border-blue-100 shadow-sm p-8 mb-6">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => setShowAddRange(false)} className="px-6 py-2 bg-slate-100 text-slate-500 rounded-lg font-bold text-sm hover:bg-slate-200 transition-all">取消</button>
            </div>
          </div>
        )}
                    ...
                    <>
                      ...
                      <td className="py-3 px-3 text-center">
                        <div className="flex justify-center gap-1">
                          ...
                          <button onClick={() => setEditingRangeId(null)} className="px-3 py-1.5 bg-slate-100 text-slate-500 rounded-lg font-bold text-xs hover:bg-slate-200">取消</button>
                        </div>
                      </td>
                    </>
                  ) : (
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[300] flex items-center justify-center p-6" onClick={() => setShowEiModal(false)}>
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl max-h-[80vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100">
              ...
              <button onClick={() => setShowEiModal(false)} className="p-2 rounded-xl hover:bg-slate-100 transition-all">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm mx-4">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button onClick={() => { confirmModal.onConfirm(); closeConfirm(); }}
                className="px-5 py-2.5 bg-rose-600 text-white rounded-xl font-bold hover:bg-rose-700 transition-all">
                確定刪除
              </button>
            </div>
          </div>
